...
或者 [jarocheng0123/yeshen.com: 夜神模拟器 root 操作指南](https://github.com/jarocheng0123/yeshen.com) 我用这个最后比较顺利搞定了。
...
下载MT 管理器，找到左侧 提取APK 功能

![image-20260630124749723](https://cdn.jsdelivr.net/gh/violet-wdream/Drawio/PNG/202606301247789.png)

找到对应app即可。

<img src="https://cdn.jsdelivr.net/gh/violet-wdream/Drawio/PNG/202606301248365.png" alt="image-20260630124821290"  />
...
启动游戏，会在`/data/user/0/GamePackageName/files/`或者`/data/data/0/GamePackageName/files/`目录下生成`dump.cs`
...
对于global-metadata.dat和libil2cpp.so都正常的情况：会生成以下这些文件，这些东西是给IDA用的。
...
命令行有点麻烦，可以找个GUI或者用我自己用的这个

 [il2cppDumper_GUI.py](https://github.com/violet-wdream/.Scripts/blob/main/Utils/il2cppDumper/il2cppDumper_GUI.py)

需要把exe路径修改成自己的

<img src="https://cdn.jsdelivr.net/gh/violet-wdream/Drawio/PNG/202606301253245.png" alt="image-20260630125328194" style="zoom:50%;" />
...
[Project SEKAI 逆向 - 笔记归档 | mos9527](https://mos9527.com/posts/pjsk/archive-20240105/#1-解密-metadata)

![img](https://cdn.jsdelivr.net/gh/violet-wdream/Drawio/PNG/202606262316425.png)

### so/dll

PC端

1. 可以用[Allycs](https://github.com/micREsoft/Allycs)运行时dump出dll
...
1. libil2cpp.so中搜索字符串`global-metadata.dat`/ `meta `/ `ERROR: Could not open %s`/ `mscorlib.dll` /  `4.0`或者搜索其中的子串，寻找相关处理方法。
...
2. 处理链路：
   ...
   ```c
   il2cpp_init -> il2cpp::vm::Runtime::Init
   ...
   1. 通常用法`il2cpp_init("IL2CPP Root Domain"); `所以可以搜索字符串`IL2CPP Root Domain`定位到` il2cpp::vm::Runtime::Init`，这里是 `sub_18016C4C0`

      会有以下特点，版本号/库文件/类型声明等，这里可以通用搜索的字符串`4.0` / `mscorlib.dll` / `Object` / `Void` …

      <img src="https://cdn.jsdelivr.net/gh/violet-wdream/Drawio/PNG/202606261707743.png" alt="image-20260626170709555" style="zoom: 33%;" />

   2. 接下来寻找`MetadataCache::Initialize`，可以在`4.0` 和 `mscorlib.dll` 这两个串中间部分的某一个函数找到，数量不多，可以逐个尝试。

      <img src="https://cdn.jsdelivr.net/gh/violet-wdream/Drawio/PNG/202606261739781.png" alt="image-20260626173900739" style="zoom: 50%;" />

      有趣的是这里有一个if判断部分`sub_18018D550`，恰好就是我们要找的目标，从工程角度来看，初始化函数需要返回操作是否成功，成功则后续代码执行，否则抛出异常。

      这个函数的特点就是会有大量的`qword`内存指针出现：

      <img src="https://cdn.jsdelivr.net/gh/violet-wdream/Drawio/PNG/202606261752367.png" alt="image-20260626175209318" style="zoom: 50%;" />

   3. 最后找`MetadataLoader::LoadMetadataFile`，谜底就在谜面上：

      注意看第一行`  result = sub_1801C1B80(&dword_181BDEC50, &dword_181BDEC60);`这里的返回结果`result`直接决定了后续代码的执行，同上一步的分析逻辑，所以这里`sub_1801C1B80`就是元数据加载函数，

      通常这里应该能直接搜索到`global-metadata.dat`，但是如果混淆/加密了就不能，但是一定能找到`ERROR: Could not open %s` 的异常判断（通常不会修改）。 

      因为这里不仅仅是混淆，还包裹了一层`sub_18016DDC0`，这里才是真正的`MetadataLoader::LoadMetadataFile`
      
      ![image-20260626182115391](https://cdn.jsdelivr.net/gh/violet-wdream/Drawio/PNG/202606261821453.png)

   查看`sub_18016DDC0`，发现 `Metadata` 以及 `ERROR: Could not open %s`

```c
__int64 __fastcall sub_18016DDC0(unsigned int *YQSY){
    //...
    v16[0] = "Metadata";
    //...
    if ( v24 )
    {
        ...
        if ( si128.m128i_i64[1] > 0xFuLL )
            v11 = (const char *)v17;
        sub_180154160("ERROR: Could not open %s", v11);
    }
}
```
...
1. Frida动态dump，PC端和移动端均可使用。 [DumpMetadata.js](https://github.com/violet-wdream/.Scripts/blob/main/Frida/DumpMetadata.js)



一些攻略

1. [[原创\]纯静态解决遇到的各类global-metadata加密问题-Android安全-看雪安全社区｜专业技术交流与安全研究论坛](https://bbs.kanxue.com/thread-289395.htm)
2. [IL2CPP 教程：寻找混淆 global-metadata.dat 文件加载程序——代码与逆向工程的冒险 --- IL2CPP Tutorial: Finding loaders for obfuscated global-metadata.dat files – Adventures in code and reverse engineering](https://katyscode.wordpress.com/2021/02/23/il2cpp-finding-obfuscated-global-metadata/)



市面上的对抗策略

1. [hybridclr](https://github.com/focus-creative-games/hybridclr)

   [代码加固 | HybridCLR](https://www.hybridclr.cn/docs/7.8.1/business/basicencryption)

2. [How Games Counter IL2CppDumper Reverse Engineering](https://www.jikguard.com/blog/how-games-counter-il2cppdumper-reverse-engineering/)

3. [Zygisk-IL2CppDumper Detection Solution](https://www.jikguard.com/blog/zygisk-il2cppdumper-detection-solution/)
...
il2cppdumper处理dll的时候最好直接使用游戏目录的`GameAssembly.dll`因为这里会引用其他库(`baselib.dll`) ，metadata没有加密。

常规导入IDA后，发现没有.text 以及其他字段，尝试复原函数名也出现了很多报错，发现dll被套壳了

<img src="https://cdn.jsdelivr.net/gh/violet-wdream/Drawio/PNG/202603101102401.png" alt="image-20260310110202278" style="zoom:50%;" />

查看`DllEntryPoint`，看不太懂，丢给llm，大致得出两个方法：

1. 手动脱壳，虽然以及得出了大部分的步骤，但是由于需要对齐/定位数据，有点麻烦，而且不太稳定。
2. 动态dump出脱壳后的dll，同样需要一定经验。

动态dump，通常可以绕过壳

<img src="https://cdn.jsdelivr.net/gh/violet-wdream/Drawio/PNG/202603102205709.png" alt="image-20260310220537646" style="zoom:50%;" />

`Allycs` 动态dump出了一个新的`GameAssembly.dll` 原来的体积是20M，这个新的是60M，同样导入IDA，尝试还原函数名，成功找到了 `UnityEngine_AssetBundle$$SetAssetBundleDecryptKey` 

<img src="https://cdn.jsdelivr.net/gh/violet-wdream/Drawio/PNG/202603102053068.png" alt="image-20260310205351992" style="zoom:50%;" />

查找引用，发现唯一的引用是`Z1ClientUpdater_GameUpdater__Awake`
...
找到`Z1ClientUpdater_GameUpdater__xlua_get_key`

反正就是通过`xlua.dll`的`xlua_get_key`

所以还需要反编译`xlua.dll`，这个也加壳了，但是没有关系，还是可以用`Allycs`继续动态dump出来

顺利找到`514/wedsjjhfb#0v`  -> HEX `3531342f776564736a6a686662233076`

但是到这里还没有结束，我测了一下还是不行。

![image-20260310212813410](https://cdn.jsdelivr.net/gh/violet-wdream/Drawio/PNG/202603102128463.png)

后面发现是因为团结引擎的包结构不太一样，key倒是对了。

[忘却前夜有大佬来解一下吗，24年的帖子方法用不了了 - 讨论 - Live2DHub](https://live2dhub.com/t/topic/5762/19)
...
2. ` cocos2d::FileUtils::decryptQD(uchar *,int,long *)`

   ```python
   QWDREAM 6B | Seed 8B | (14B)
   EncryptedData (n - 14) B
   ```

   `Index = (13031249 * ((Seed+ 28800000) % 0x5265C00)) >> 32)  >> 15`

   `0x5265C00 = 86400000 = 24 * 60 * 60 * 1000`
...
3. `cocos2d::FileUtils::addSeasoning(EncryptedData, Key)`
...
3. astc → png，astcenc 这玩意有点慢，后面换个多线程的试试 [Astc2Png.py](https://github.com/violet-wdream/.Scripts/blob/main/Png/Astc2Png.py)
