...
可以看到最底下的注释也是`global_metadata.dat`，但是这里的魔数并不是的 `AF 1B B1 FA`，不管了先dump出来再说。

打开文件夹里的元数据发现和内存里的一模一样，反应过来这个全局变量就是未被处理的元数据。
...
移动端没有加密，后面发现这个游戏作者魔改了live2d的一些东西。

[求解惑，单机游戏提取动画资源 - 讨论 - Live2DHub](https://live2dhub.com/t/topic/5184/7)
...
<img src="https://cdn.jsdelivr.net/gh/violet-wdream/Drawio/PNG/202606291608393.png" alt="image-20260629160850270" style="zoom: 50%;" />
...
``` 
cocos2d::FileUtilsAndroid::getData
cocos2d::FileUtils::decryptData
cocos2d::FileUtils::loadFilenameLookupDictionaryFromFile
cocos2d::FileUtils::setFilenameLookupDictionary
cocos2d::FileUtils::getJZLocalFileName
```
