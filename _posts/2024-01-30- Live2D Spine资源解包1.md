...
[来自蝙蝠侠](https://live2dhub.com/t/topic/3741)

[resdownloader - Software Download](https://resdownload.7sec.fun/resdownloader/)  资源多，要手动设置代理，跟模拟器代理一个操作。代理软件开局域网连接，ipconfig查一下局域网的IPv4拼接一下代理端口，EroLab基本上就那几个头牌能看。
...
4. PtN版本[无期迷途 - Google Drive](https://drive.google.com/drive/folders/1J6XrLk0rkyBBTs832AMC4qTWvGluPyNH)：无期迷途特供版，来自Live2DHub网友Yjzy，解了FakeHeader和UnityCN同时疑似破解了hash参数。百度网盘链接: https://pan.baidu.com/s/18dJhlonBf2YPKjLIqDGMbw 提取码: njrj 
...
```bash
/Android/data/com.bilibili.azurlane/files/AssetBundles/spinepainting/
```
...
模型json处理脚本(特定格式化model3.json文件，可以使用OML2D预览)
...
[GetHashCSV](https://github.com/violet-wdream/.Scripts/blob/main/Games/Azurlane/GetHashCSV.py)

根据版本更新，这些哈希值都会改变，需要即时获取。值得一提的是以前的也可以用。
...
拼接URL`l2d` `painting`，其他的同理

```nginx
https://line3-patch-blhx.bilibiligame.net/android/hash/$l2dhash$1337$3897396067834295
https://line3-patch-blhx.bilibiligame.net/android/hash/$l2dhash$1342$16f09ca3bf0dc5a1
https://line3-patch-blhx.bilibiligame.net/android/hash/$l2dhash$1354$0f510c8134efe70a

https://line3-patch-blhx.bilibiligame.net/android/hash/$paintinghash$1428$84e19a0b1737a37e

https://line3-patch-blhx.bilibiligame.net/android/hash/$azhash$9$6$523$3880170b84276e2f
```



（可选）下载完后对清单进行排序，排序规则是文件名的字典序

[ProcessHashCSV](https://github.com/violet-wdream/.Scripts/blob/main/Games/Azurlane/ProcessHashCSV.py)



清单每一行内容是`filename,size,MD5`
...
![image-20260219231851393](https://cdn.jsdelivr.net/gh/violet-wdream/Drawio/PNG/202602192318507.png)

![image-20260219231814110](https://cdn.jsdelivr.net/gh/violet-wdream/Drawio/PNG/202602192318234.png)

2026-2-8 测试下载 高雄 皮肤，测试成功。
...
他这个更新不是碧蓝航线那个，按时间排序一下就知道哪个是新的了，所有的模型都绑定在了同一个文件。。。
...
[DecryptBundles.py](https://github.com/violet-wdream/.Scripts/blob/main/Games/PathToNoWhere/DecryptBundles.py)

打开解密后，有些文件可以看到是FakeHeader
...
[Live2DHub](https://live2dhub.com/t/topic/3342/4)

**可以直接导出raw数据，然后删除头部的脏数据，使文件头为MOC，然后修改拓展名moc3即可使用，不用导出json然后转moc3。**

如果非要导出json，使用脚本提取moc3文件（把json文件的_bytes数组合并**合成完整的二进制文件**）。
...
? -> Parameters –hash–> hashTable<hash, pid>

fadeMotionlist -> pid -> fade -> motion3 (hash) –hashTable–> motion3  
...
https://live2dhub.com/t/topic/6005/1

2026.5.14更新：

使用AS版本为[aelurum/AssetStudio: AssetStudioMod](https://github.com/aelurum/AssetStudio)
...
因为大部分容器路径被破坏了，所以处理步骤十分繁琐。



1. 导出动作，搜索`.fade`导出所有模型的所有`fade`动作，以及`.fadeMotionList`到 Fade 目录

   注意这里需要修改导出设置，因为需要pathID来进行分类。

   ![image-20260516164351007](https://cdn.jsdelivr.net/gh/violet-wdream/Drawio/PNG/202605161643089.png)

2. 导出贴图，搜素`texture_0`，筛选文件类型`Texture2D`导出到Textures目录，导出设置同步骤1。

3. 导出贴图元数据，正则匹配搜索`.*\.(1024|2048|4096)\.bundle$`，筛选文件类型`AssetBundle`导出到 TexturesMeta 目录，导出设置

   ![image-20260516164457644](https://cdn.jsdelivr.net/gh/violet-wdream/Drawio/PNG/202605161644718.png)

   ![image-20260516143700531](https://cdn.jsdelivr.net/gh/violet-wdream/Drawio/PNG/202605161437648.png)

4. 导出moc3，筛选文件类型`MonoBehaviour (Live2D Model)`，导出到 Moc3Json 目录，导出设置同步骤3

   <img src="https://cdn.jsdelivr.net/gh/violet-wdream/Drawio/PNG/202605161545412.png" alt="image-20260516154506250" style="zoom: 67%;" />

5. moc3转换，读取 Moc3Json 目录把json转换为moc3格式，同时输出到 Output 目录下同名模型目录。

6. fade动作转换，读取 Fade 目录把所有的fade动作转换为对应的json格式，也就是`.motion3.json`，同时根据`.fadeMotionList`将转换后的动作输出到 Output 目录下同名模型目录（的 motions 子目录）。

7. 贴图分类，通过解析元数据获取pathID，然后通过pathID将相应贴图输出到 Output 目录下同名模型目录（的 textures 子目录）。

8. 遍历各个 Output 目录下模型目录，在其根目录下生成`.model3.json`
...
![image-20260518235320808](https://cdn.jsdelivr.net/gh/violet-wdream/Drawio/PNG/202605182353928.png)
...
但是目前没有具体的解析办法，但是值得一提的是：可读的部分明显都是资产文件，所以只要把可读的部分提取出来即可（不确定是否会缺失内容）。可以尝试AddressbleToolspy。
...
游戏版本号 `game_config_patch > extra > hotfix_version > 2.3.48`

cdn链接`https://xonecn-hotupdatecdn.shziyi.com` 或者 `https://xonecn-hotupdatecdn-backup.shziyi.com`
...
这里请求的URL里面`2.3`应该是大版本号，因为`"apk_version": "2.3.2", "hotfix_version": "2.3.48"`的大版本号都是2.3。同时最后的版本号48也就是`2.3.48`的后面这个小版本号48。
...
下载的是.patch文件，且所有patch都下载完后就会进行解压操作。实测直接用解压软件就可以打开
...
可以看到这里写明了secretJson，说明大概率与被加密的json有关。
...
[/PathToNoWhere/Decrypt](https://github.com/violet-wdream/.Scripts/blob/main/Games/PathToNoWhere/Decrypt.py)
...
1. 构造请求获取`patchlist`，得到APK链接、游戏本体Version（比如2.3.48，取前两位2.3）
2. 解压APK得到一部分静态资源，以及`res_base.json`（无法单独获取，放在了APK里），解密`res_base.json`得到明文，依次拼接baseURL得到所有基础资产。
...
`assets/rawres/effect/ugui/spine/summertittle/`

`assets/rawres/effect/ugui/spine/`

`assets/rawres/effect/ugui/skillspine/`
...
[RequestVersion.py](https://github.com/violet-wdream/.Scripts/blob/main/Games/RiichiCity/RequestVersion.py)

```json
{"ApkSize":0,"code":0,"data":{"AliHotUpdateUrl":"https://happywoods-android1.oss-cn-hangzhou.aliyuncs.com/android/20260209003","ApkSize":0,"ApkUrl":"https://play.google.com/store/apps/details?id=com.riichicity.happywoods","Country":"CN","DownApkUrl":"https://happywoods-dev.oss-cn-shenzhen.aliyuncs.com/apks/official_prod_2.2.2_245.apk","ForbidDomain":[],"HotUpdateUrl":"https://happywoods-android1.oss-cn-hangzhou.aliyuncs.com/android/20260209003","IsDefaultDomain":false,"MainlandDomain":"cn2aws.riichimj.com","MinVersion":"86097","Version":"2.2.2"},"message":"ok"}
```

通过`AliHotUpdateUrl` 拼接得到`spine_version.txt`

```
https://happywoods-android1.oss-cn-hangzhou.aliyuncs.com/android/20260209003/spine_version.txt
```

 拼接得到`spine_res_bundle_data.dat`

```
https://happywoods-android1.oss-cn-hangzhou.aliyuncs.com/android/20260209003/spine_res_bundle_data.dat
```
...
## 放置少女（HouchiShoujo） Spine/Live2D - RC4
...
后面发现了过完新手引导之后，在设置里面可以手动下载所有的额外资源。

<img src="https://cdn.jsdelivr.net/gh/violet-wdream/Drawio/PNG/202602081628126.png" alt="image-20260208162807933" style="zoom:50%;" />

<img src="https://cdn.jsdelivr.net/gh/violet-wdream/Drawio/PNG/202602081629127.png" alt="image-20260208162906014" style="zoom:50%;" />



### 新增更新器

使用脚本[HouChiShouJo](https://github.com/violet-wdream/.Scripts/tree/main/HouChiShouJo)
...
metadata是加密的，可以通过frida动态获取。

视频/音频 `CriWareDecrypter__Initialize` -> `CriWareDecrypter__CRIWAREFED2AA49`

```c
realKey = userKey ^ 0xD47EB533AEF7E5
```

ab `LoveEngine_Native_CompressUtils__RC4_DecryptBufferOverlapped_38457040` -> ` LoveEngine_Native_CompressUtils__RC4_encrypt` -> `__fastcall sub_9E9688 ` -> `libLoveNI.so` -> `fsni_open ` -> `loveni_AES_encrypt`
...
{
    AES_set_encrypt_key((void *)s_default_secret, 128, v38);
    *(_OWORD *)(v38 + 244) = *(_OWORD *)s_default_iv;
    *((_QWORD *)fileName_1 + 3) = v38;
}
```

![image-20260214131026544](https://cdn.jsdelivr.net/gh/violet-wdream/Drawio/PNG/202602141310617.png)

![image-20260214130952356](https://cdn.jsdelivr.net/gh/violet-wdream/Drawio/PNG/202602141309509.png)
...
## 未完待续==最后的起源(Last Origin) UnityAnim/Spine 无加密 路径明确
...
<img src="https://lastorigin.wiki.gg/images/thumb/Kunoichi_Zero_Skin_1.png/1024px-Kunoichi_Zero_Skin_1.png?a6c8b2" alt="Kunoichi Zero Skin 1.png" style="zoom: 33%;" />

<img src="https://lastorigin.wiki.gg/images/thumb/Kunoichi_Zero_Skin_1_Damaged.png/800px-Kunoichi_Zero_Skin_1_Damaged.png?dd6d0e" alt="Kunoichi Zero Skin 1 Damaged.png" style="zoom: 33%;" />

参考：

1. [有没有大佬提取下last origin的 - 讨论 - Live2DHub](https://live2dhub.com/t/topic/443/15)
2. [Last Origin アセット抽出メモ - mizle notes](https://scrapbox.io/mizle/Last_Origin_アセット抽出メモ)

Wiki

[Last Originwiki官网_Last Origin图鉴|GameKee](https://www.gamekee.com/lo/)



### 资源

1. 韩服（官服）[Latest Korean Apk - Last Origin Wiki](https://lastorigin.wiki.gg/wiki/Latest_Korean_Apk) 和谐，进度(2.6.07)最领先。
2. 日服，和谐，进度比韩服慢一点。
3. 台服（pmang服）[【公式】LAST ORIGIN R+ | Pmang(G・O・P運営)](https://lastorigin-r.pmang.jp/ja/download/apk)无和谐，进度(1.4.0)落后一个大版本
4. Erolab服[EROLABS | Best Waifu of Your Laifu](https://game.erolabs.online/game.html?id=104)  无和谐，进度(1.4.0)落后一个大版本

pmang服注册很麻烦，erolab服可以游客登陆。
...
<img src="https://cdn.jsdelivr.net/gh/violet-wdream/Drawio/PNG/202511261437352.png" alt="image-20251126143748197" style="zoom:50%;" />
...
2. _ndl - ?
3. _dl - ?
4. _n / _ns{number}  - 常规形态/皮肤

### 检测绕过

使用了Inka-AppSealing 进行软件检测，包括但不限于`MT管理器` `LSPosed`，如果使用Blocker强行禁止，软件会自动闪退。

![image-20260624200044875](https://cdn.jsdelivr.net/gh/violet-wdream/Drawio/PNG/202606242000181.png)

尝试了一圈各种方法，目前发现最简单方法，直接用app cloner复制一份（#N1 ），然后启动这个副本就自动绕过检测了，然后就可以把原版的删除了。原理类似是环境隔离？

<img src="https://cdn.jsdelivr.net/gh/violet-wdream/Drawio/PNG/202606242053300.png" alt="image-20260624205322153" style="zoom:50%;" />

```
(py314) PS D:\Games\GameUnpackAssets\mymodel\.Scripts> adb shell pm path --user 10 com.smartjoy.LastOrigin_G
package:/data/app/~~JdBV5oA4il3Sgx3Lr2Maqw==/com.smartjoy.LastOrigin_G-NXGOeYFcrBcjylnweATg7w==/base.apk
package:/data/app/~~JdBV5oA4il3Sgx3Lr2Maqw==/com.smartjoy.LastOrigin_G-NXGOeYFcrBcjylnweATg7w==/split_config.arm64_v8a.apk
```

这个克隆是通过切换用户实现的，默认用户是0，克隆使用用户10，所以文件并不在常规路径下。

![image-20260625001620966](https://cdn.jsdelivr.net/gh/violet-wdream/Drawio/PNG/202606250016096.png)

经过一番查找，最后找到位置：

韩服

`/data/media/10/Android/data/com.smartjoy.LastOrigin_G/`

erolab服

`/data/media/10/Android/data/com.valofe.laotw.ero/`
...
<img src="https://cdn.jsdelivr.net/gh/violet-wdream/Drawio/PNG/202512051940413.png" alt="image-20251205194006174" style="zoom:50%;" />

导入提取器，选择portrait导出

<img src="https://cdn.jsdelivr.net/gh/violet-wdream/Drawio/PNG/202512052013377.png" alt="image-20251205201358294" style="zoom:50%;" />

右下角两个按钮是插件，默认打开不用关。

### scsp转换

[第七史诗(EpicSeven) Scsp3.8.99 解析 json骨骼](https://live2dhub.com/t/topic/5799/6)
...
LZ4 compressed data:
    ...
    binary_data | (data_size)B
        ...
        HeaderSize 4B (min(headerSize, 88))
        Header (HeaderSize)B 
...
<img src="https://cdn.jsdelivr.net/gh/violet-wdream/Drawio/PNG/202603201406811.png" alt="image-20260320140615487"  />

![image-20260320141700111](https://cdn.jsdelivr.net/gh/violet-wdream/Drawio/PNG/202603201417279.png)
...
1. E7 Ripper: get spine files (png, scsp, atlas)
2. atlas : .sct -> .png
3. scsp -> json
...
```c
http://dqssupdate.zlongame.com/DQSS/online/update/media_1369/_media_pre_1369_9a8cf3992e034ff57649d0fb6809a4cd.pcarch
http://dqssupdate.zlongame.com/DQSS/online/update/res_1376/_res2_1373_9d7dfe3100e6639543ec7d5d9e367d13.pcarch
http://dqssupdate.zlongame.com/DQSS/online/update/text_1373/_text_zhs_1373_b64db5582537bd768a6880b0c7cf8522.pcarch
```

<img src="https://cdn.jsdelivr.net/gh/violet-wdream/Drawio/PNG/202603021213283.png" alt="image-20260302121335189" style="zoom:50%;" />
...
```json
https://version-asiangame2.gtarcade.com/opinfo?opGameId=2752&opId=2226&channel=0&md5=

https://cdn-wx-studio.gtarcade.com/product-2002003/cdn1/main/update/inner/1.6.75/10030_10675_android_0.zip
https://cdn-wx-studio.gtarcade.com/product-2002003/cdn1/main/update/inner/1.6.87/10681_10687_android_0.zip
https://cdn-wx-studio.gtarcade.com/product-2002003/cdn1/main/update/inner/1.6.75/10030_10675_android_1.zip
```
...
```c
http://p33-jp-prd-main.jedi-games.com:9090/p33/main/checkver?func=checkver&platform=googleplay&channel=googleplay&os=android&ver=1.28.12&account=399697&original_ver=1.28.0.41161&deviceid=78a3c98b-527d-43a6-b7e5-9c4d6f5cd4d6
```

会得到BaseURL`http://p33-obt-cdn.jedi-games.com/hot_test/android/1.28.17.42182`

```json
{"is_censor":0,"force":1,"url_hotfix":"http://p33-obt-cdn.jedi-games.com/hot_test/android/1.28.17.42182","type":1,"ret_code":0,"newver":"1.28.17","md5":"42182"}
```
...
```c
#资产清单
http://p33-obt-cdn.jedi-games.com/hot_test/android/1.28.17.42182/_files.txt
#更新内容
http://p33-obt-cdn.jedi-games.com/hot_test/android/1.28.17.42182/newExpansionPackageInfo.txt
```
